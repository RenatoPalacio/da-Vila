...
class _LercarrinhoState extends State<Lercarrinho> {
  ...
  List<String> _deliveryincorreto = List();
  ...
  _checarInfosLoja() async {
    ...
    for(DocumentSnapshot item in _itensCarrinho.documents){
      ...
      _valorminimo = item["valorminimo"];
      if (_valorminimo > 0){
        double _total = item["valor"] * item["quantidade"];
        _valorPedido = _valorPedido + _total;
      }

      //checa se loja aceita o delivery definido pelo usuário
      _retirada = item["retirada"];
      _fazentrega = item["fazentrega"];

      if(_enderecoEntrega == "retirada" && !_retirada || _enderecoEntrega != "retirada" && !_fazentrega){
        _deliveryincorreto.add(item.documentID);
      } else {
        _listadelivery.add({
          "data": _dataagora,
        });
    ...
    //confere o último registro da tabela Carrinho para o valor mínimo
    if(_valorPedido < _valorminimo){
      _lojassemmin.add(_nomeLoja);
    }
    //_lojassemmin.clear();
    int _tamanhoValorMin = _lojassemmin.length;
    int _tamanhoDlvryIncorreto = _deliveryincorreto.length;
    print(_tamanhoDlvryIncorreto);
    if(_tamanhoValorMin > 0){
      String _texto = "Você não atingiu o valor mínimo da loja.";
      _checarAlerta(_tamanhoValorMin, _qtd_lojas, _texto);
    } else if(_tamanhoDlvryIncorreto > 0){
      String _comptexto;
      _enderecoEntrega == "retirada" ?  _comptexto = "aceita retirada. Defina um endereço de entrega no topo desta tela ou cancele a encomenda." : _comptexto = "faz entrega. Altere a forma para Retirada no topo desta tela ou cancele a encomenda.";
      String _texto = "Esta loja não $_comptexto";
      _checarAlerta(_tamanhoValorMin, _qtd_lojas, _texto);
    }
    else {
      _fazerPedido();
  ...
  _checarAlerta(int _tamanho, int _qtd_lojas, String _texto){
    /*
    if (_qtd_lojas == 1){
  ...
  _cancelarEncomendas() async {
    db = Firestore.instance;
    for(String _idProduto in _deliveryincorreto){
      db.collection("usuarios")
          .document(_idUsuario)
          .collection("carrinho")
          .document(_idProduto).delete();
                                      ...
                                      indice == _tamanhoLista - 1
                                        ...
                                        children: <Widget>[
                                                  ...
                                                  child: BotaoCustomizado(
                                                      ...
                                                      onPressed: (){
                                                        if(_enderecoEntrega != null){
                                                          // checa valor mínimo e modalidade delivery da loja
                                                          _checarInfosLoja();
                                                        } else{
                                                          showDialog(
                                                              context: context,
                                          ...
                                          Column(
                                            children: <Widget>[
                                              Padding(
                                                padding: EdgeInsets.fromLTRB(10, 64, 0, 10),
                                                child: RaisedButton(
